...
class Plotter {
    constructor() {
        ...
        this.font = {
            ...
            size: commander.fontSize ||
                ((this.outputType === outputGraph.PNG) ? 14 : 18),
        };

        if (this.outputType === outputGraph.PDF) {
            this.terminalType = 'pdfcairo';
            this.outputExt = '.pdf';
            this.graphSize = [17, 11]; // in inches of A4 paper
            const coef = (this.font.size > 20) ? 0.4 : 0.2;
            this.extraSize = [0, coef * this.graphSize[1]];
        } else {
            this.terminalType = 'pngcairo';
            this.outputExt = '.png';
            this.graphSize = [1280, 720]; // in pixels of HDV resolution
            const coef = (this.font.size > 20) ? 1 : 0.5;
            this.extraSize = [0, coef * this.graphSize[1]];
        }
        ...
        this.ssm = commander.ssm || s3Config.simul.ssm ||
            objFromS3blaster.arrDataFiles[dataFiles.ssm];
        this.liveGlobal =
    ...
    storeResultS3blaster(cb) {
        ...
        const content = JSON.stringify(objFromS3blaster, null, 4);
    ...
    genTerminal(nbX, nbY, ratio, _extra) {
        let width = this.graphSize[0];
        let height = this.graphSize[1];
        const extra = _extra || this.extraSize;
        if (ratio) {
            width *= ratio[0];
            height *= ratio[1];
        }
        return `${this.terminalType} size ` +
               `${width * nbX + extra[0]},${height * nbY + extra[1]} `;
    }
    ...
    *  latency and #operations/s vs. data sizes
    ...
    createGnuFileThread(cb) {
        ...
        if (this.paramsForSummaryPlot && this.paramsForSummaryPlot.inputFile) {
            inputFile = this.paramsForSummaryPlot.inputFile;
        }
        if (this.paramsForSummaryPlot && this.paramsForSummaryPlot.gnuFile) {
            gnuFile = this.paramsForSummaryPlot.gnuFile;
        }
        if (this.paramsForSummaryPlot && this.paramsForSummaryPlot.outputFile) {
            outputFile = this.paramsForSummaryPlot.outputFile;
        }
        if (this.paramsForSummaryPlot && this.paramsForSummaryPlot.stdDevPlot) {
            stdDevPlot = (this.paramsForSummaryPlot.stdDevPlot !== 'dontPlot');
        }

        const xticsArr = [];
        if (this.paramsForSummaryPlot && this.paramsForSummaryPlot.nProcesses) {
            this.threads.forEach(thr => {
                const _thr = thr * this.paramsForSummaryPlot.nProcesses;
                xticsArr.push(`"${_thr}" ${_thr}`);;
            });
        } else {
            this.threads.forEach(thr => {
                xticsArr.push(`"${thr}" ${thr}`);;
            });
        }
        const xticsStr = xticsArr.join(', ');
        ...
        this.reqsToTest.forEach((req, reqIdx) => {
            ...
            const sizesStr = this.sizes.map(size => convertSize(size)).
                                join(', ');
            let title = `Average latency, ${reqsString[req]}, ` +
                        `object sizes = ${sizesStr}`;
            content += `unset title; set title "${title}"\n`;
            content += 'plot ';
            this.sizes.forEach((size, idx) => {
                ...
                content += this.plotLine(inputFile,
                    colsToPlot, [firstLine, step], legend, 'linespoints',
            ...
            title = `Average #operations/s, ${reqsString[req]}, ` +
                        `object sizes = ${sizesStr}`;
            content += `unset title; set title "${title}"\n`;
            content += 'plot ';
            this.sizes.forEach((size, idx) => {
        ...
        this.reqsToTest.forEach((req, reqIdx) => {
            ...
            this.sizes.forEach(size => {
                ...
                const title = `Average #operations/s, ${reqsString[req]}, ` +
                            `object size = ${convertSize(size)}`;
                content += `unset title; set title "${title}"\n`;
                content += 'plot ';
                if (this.fitPlot && this.threads.length > 4) {
                    content += this.plotLine(inputFile,
        ...
        this.reqsToTest.forEach((req, reqIdx) => {
            ...
            this.sizes.forEach(size => {
                ...
                const title = `Average latency, ${reqsString[req]}, ` +
                            `object size = ${convertSize(size)}`;
                content += `unset title; set title "${title}"\n`;
                content += 'plot ';
                if (this.fitPlot && this.threads.length > 4) {
