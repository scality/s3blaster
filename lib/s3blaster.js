...
// get root folder of s3blaster
let statsFolder = __dirname.slice(0, __dirname.length - 3);
statsFolder += `results/${_folderName}/`;
...
function helpS3blaster() {
    ...
    stdout.write('  --fill-objs: flag for filling objects in buckets\n');
    stdout.write('  --fill-range: Indices range for filling objects min:max\n');
...
class S3Blaster {
    constructor(host, port) {
        ...
        .option('--fill-objs [fillObjs]',
            'Flag for filling objects in each bucket')
        .option('--fill-range [fillRange]',
                    'Indices range for filling objects min:max')
        .option('--fill-threads [fillThreads]',
        ...
        .option('--work-onCurrObjs [workOnCurrObjs]', 'Work on uploaded objs')
        .parse(process.argv);
        ...
        this.fillObjs = (commander.fillObjs || s3Config.db.fillObjs) === 'yes';
        this.fillRange = commander.fillRange || s3Config.db.fillRange;
        if (this.fillRange) {
            this.fillRange = this.fillRange.split(':').map(Number);
        } else {
            this.fillRange = ['', ''];
        }
        this.fillThreads = commander.fillThreads || s3Config.db.fillThreads ||
                                32;
        ...
        this.simulDelay =
            (commander.simulDelay || s3Config.simul.simulDelay || 0) * 1000;
        ...
        this.workOnCurrObjs = (commander.workOnCurrObjs ||
                                s3Config.simul.workOnCurrObjs);
        this.workOnCurrObjs = (this.workOnCurrObjs === 'yes');
    ...
    printParams() {
        ...
        info = info.concat([
            `Parallel requests nb per worker (#PR): ${this.rThreads}`,
        ...
        this.reqsToTest.forEach((req, idx) => {
            const str = toFixedLength(requests[req], 15, 'left') +
                toFixedLength(this.reqRates[idx], 30, 'right') +
                toFixedLength(this.delays[req], 15, 'right') +
                toFixedLength(this.fracLoads[idx].toFixed(2), 15, 'right') +
                toFixedLength(this.rAll[req], 15, 'right');
    ...
    checkConsistentParams() {
        ...
        if (this.fillRange && !this.fillObjs) {
            if (this.fillRange[1] > this.fillRange[0] &&
                this.fillRange[0] > 0) {
                this.fillObjs = true;
            }
        }
        ...
        if (cluster.isMaster) {
            ...
            objToPlotter.divisionMarks = this.rThreads.map(() =>
    ...
    printStats(idx, _sizeIdx) {
        ...
        if (this.percentiles) {
            ...
            valuesToPlot =
                valuesToPlot.concat(this.percentilesArr[idx][sizeIdx]);
        }
    ...
    fillObjects(cb, _keyMin, _keyMax, _threadsNb) {
        ...
        const threadsNb = _threadsNb || this.fillThreads ||
                                Math.max.apply(Math, this.rThreads);
    ...
    doSimul(cb) {
        ...
        this.currIndx = requests.map((req, reqIdx) => this.rAll[reqIdx][0]);
            ...
            if (cluster.isMaster) {
                for (let i = 0; i < this.nWorkers; i++) {
                    cluster.fork().on('message', msg => {
                        ...
                        if (msg && msg.time > objToPlotter.divisionMarks
                            [msg.thread][msg.size][msg.request]) {
        ...
        if (this.count >= this.threshold || runTime > this.runTime) {
            ...
            if (this.threads === 0) {
                ...
                if (cluster.isWorker) {
                    if (this.simulPolicy === simulEach) {
                        process.send({
                            id: cluster.isMaster ? 0 : cluster.worker.id,
        ...
        latestStates.forEach((val, valIndx) => {
            if (val && val !== ' ') {
                ...
                if (Array.isArray(_ssmTypes[ssmTypes[valIndx %
                                                allTypesLen]])) {
                    strToFile += `${toFixedLength(' ?0/1 ?0/1 ',
                                                    serverColLen)}`;
                } else {
                    strToFile += `${toFixedLength(' ?0/1 ', serverColLen)}`;
                }
            }
        });
        if (this.displaySSM) {
    ...
    getAcl(cb, threadIdx) {
        ...
        if (this.workOnCurrObjs) {
            ...
            key = (this.getNextKey(GET_ACL, this.rGet[0], this.rGet[1]));
        }
    ...
    del(cb, threadIdx) {
        ...
        this.s3.deleteObject(object, err => {
            ...
            this.doNextAction.bind(this)(DEL_OBJ, cb, threadIdx);
        });
        return undefined;
    }
