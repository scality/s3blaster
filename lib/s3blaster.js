...
class S3Blaster {
    ...
    checkConsistentParams() {
        ...
        // update objToPlotter
        if (cluster.isMaster) {
            objToPlotter.id = `${defaultFileName}` +
                `Master_${(new Date()).getTime()}_${genRandStr(3)}.id`;
        } else {
            objToPlotter.id = `${defaultFileName}` +
                `${(new Date()).getTime()}_${genRandStr(3)}.id`;
            process.send({
