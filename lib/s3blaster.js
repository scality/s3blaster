...
function genObjs(size) {
    return size > 0 ? crypto.randomBytes(size) : Buffer.alloc(0);
}
...
class S3Blaster {
    constructor() {
        ...
        s3config.update({
            ...
            computeChecksums: false,    // NOTE: computeChecksums takes 2ms more
        ...
        this.md5s = this.values.map(val =>
            crypto.createHash('md5').update(val, 'binary').digest('hex'));
    ...
    checkConsistentParams() {
        ...
        // check this.observationsNb
        if (this.observationsNb < this.paralReqs[this.paralReqs.length - 1]) {
            this.observationsNb = this.paralReqs[this.paralReqs.length - 1];
        }
    ...
    fillObjects(cb, _keyMin, _keyMax, _threadsNb) {
        ...
        function putObj(cb) {
            ...
            const keyIndx = keyMin + (count % fillObsNb);
    ...
    put(cb, threadIdx) {
        ...
        object.Metadata = {};
        for (let i = 0; i < this.userDefinedHead[0]; i++) {
            object.Metadata[`header${i}`] = `string${i}`;
        }
        for (let i = 0; i < this.userDefinedHead[1]; i++) {
            object.Metadata[`header${this.userDefinedHead[0] + i}--integer`] = `${i}`;
        }
    ...
    mpu(cb, threadIdx) {
        ...
        let partsNb = 1;
        if (this.partSize > 0) {
            partsNb = Math.ceil(this.size / this.partSize);
        }
        ...
        function uploadPart(data, cb, _partNumber) {
            ...
            let objLen = this.partSize;
            if (partNumber === partsNb && this.size % this.partSize !== 0 &&
                this.size > 0) {
                objLen = this.size % this.partSize;
            }
    ...
    getAllData(cb, mdSize) {
        ...
        this.paralReqs.forEach(thread => {
            threadStr += `${toFixedLength(thread * this.nProcesses,
        ...
        this.paralReqs.forEach(thread => {
            this.sizes.forEach(size => {
                str += `${toFixedLength(thread * this.nProcesses, 12)} ` +
                       `${toFixedLength(size, 10)} `;
                this.reqsToTest.forEach(req => {
                    this.perfs.forEach(perf => {
                        const avg = this.getData(req, size, thread, perf);
