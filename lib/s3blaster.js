...
const ssmLogger = ssmConfig.logger;
...
mdInfo.bucketsCommonPath = servers.map(server =>
    `/scality/ssd1/ironman/scality-metadata-${server}/databases/${server}/`
);
mdInfo.bucketsList = [];
...
class S3Blaster {
    constructor(host, port) {
        ...
        if (cluster.isMaster) {
            if (this.ssm) {
                ...
                process.on('SIGINT', () => {
                    ssmLogger.info('Received SIGINT');
                    ssmSystem.terminate(() => {
                        process.exit(0);
                    });
                });
    ...
    getInitMDSize(cb) {
        setTimeout(ssmSystem.getInitMDSize.bind(ssmSystem), 100, err => {
            if (err) {
                return setTimeout(this.getInitMDSize.bind(this), 1000, cb);
            }
            return cb();
        });
    }
    ...
    calculateMDSize(cb) {
        setTimeout(ssmSystem.calculateMDSize.bind(ssmSystem), 100,
            (err, mdSize) => {
                if (err) {
                    return setTimeout(this.calculateMDSize.bind(this), 1000,
                                        cb);
                }
