...
class S3Blaster {
    ...
    printParams() {
        ...
        this.message += `${reqStr}` +
            '# Number of parallel requests per S3Blaster worker (#PR):' +
            ` ${this.rThreads}\n` +
    ...
    fillObjects(cb, objsNb, _threadsNb) {
        ...
        let _objsNb = parseInt(objsNb, 10) || 0;
        if (_objsNb === 0) {
            return cb();
        }

        _objsNb = Math.min(_objsNb, this.maxKey);
        ...
        function putObj(cb) {
            ...
            const object = {
                Key: this.genKey((count - 1) % _objsNb, this.sizes[sizeIndx]),
            ...
            return this.s3.putObject(object, err => {
                ...
                if (!this.storedKeys[bucket][sizeIndx][object.Key]) {
    ...
    genKey(keyIndx, _size) {
        ...
        return this.prefixKey ? `${this.prefixKey}_Id${keyIndx}` :
                `key_Wker${workerId}_Size${size}_Id${keyIndx}`;
    }
    ...
    get(cb, threadIdx) {
        ...
        if (this.getAnyKey) {
            ...
            if (this.keysNb[bucketName][sizeIdx] > 0) {
                const index = this.getNextKey(GET_OBJ, 0,
                    this.keysNb[bucketName][sizeIdx], null, true);
                key = Object.keys(this.storedKeys[bucketName][sizeIdx])[index];
            } else {
                if (this.noKeyFlag) {
                    key = 'undefined';
                } else {
    ...
    del(cb, threadIdx) {
        ...
        let key;
        if (this.keysNb[bucketName][sizeIdx] > 0) {
            const index = this.getNextKey(DEL_OBJ, 0,
                this.keysNb[bucketName][sizeIdx], null, true);
            key = Object.keys(this.storedKeys[bucketName][sizeIdx])[index];
        } else {
            if (this.noKeyFlag) {
                ...
                if (this.simulPolicy === simulEach) {
                    if (this.keysNb.every(arrPerBkt =>
                        arrPerBkt.every(arrPerSize => arrPerSize === 0))) {
        ...
        this.s3.deleteObject(object, err => {
            ...
            if (err && (!(this.noKeyFlag &&
                ...
                if (key !== 'undefined') {
                    // TODO: set value to be undefined make program faster
                    // this.storedKeys[bucketName][sizeIdx][key] = undefined;
