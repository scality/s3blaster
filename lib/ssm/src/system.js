...
const allServers = require('../../config.js').config.conn.servers;
            ...
            if (err) {
                ...
                if (tryNext) {
                    ...
                    this.currSSHConfig.user = readlineSync.question(
                        `Enter user[${server.username}]: `) || server.username;
            ...
            monitor.calculateMDSize.bind(monitor)((err, mdSize) => {
                ...
                if (count === this._monitors.length) {
                    ...
                    allServers.forEach(server => {
                        _mdSize[server] = mdSize[server][1] - mdSize[server][0];
                    });
                    return cb(err, _mdSize);
                }
                return undefined;
            });
        });
