...
class SSHConnection extends Connection {
     ...
     */
    memoryInfo(callback) {
        this.execute('cat /proc/meminfo', (err, data) => {
            if (err && callback) {
                callback(err, null);
            } else {
    ...
    extratInfo(input, sum) {
        ...
        return ` ${toFixedLength((res[0] / 1024).toFixed(0), 10, 'left')}` +
               `${toFixedLength(res[1].toFixed(0), 10)} `;
    }
    ...
    s3(callback) {
        const cmd = 'COLUMNS=200 top -c -b -d1 -n1 | grep "IronMan-S3"|' +
                    'awk \'{printf("%s,%s;", $6, $9);}\'';
        this.execute(cmd, (err, data) => {
            if (data) {
                return callback(err, this.extratInfo(data));
    ...
    ironman(callback) {
        const cmd = 'COLUMNS=200 top -c -b -d1 -n1 | grep ' +
            '"IronMan\\|repd\\|bucketd\\|RepdServerMap\\|supervisord\\|' +
