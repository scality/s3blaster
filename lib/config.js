...
function getSecretInfo(accFilePath) {
    ...
    if (cluster.isMaster && commander.ssm && commander.ssm === 'true') {
        ...
        if (!publicKey) {
            publicKey =
                readlineSync.question('Enter path for your publicKey: ');
            password =
                readlineSync.question('Enter password for your publicKey: ',
                    { hideEchoBack: true });
            if (!publicKey) {
                privateKey =
                    readlineSync.question('Enter path for your privateKey: ');
                passphrase =
                    readlineSync.question('Enter passphrase for privateKey: ',
                        { hideEchoBack: true });
            }
        }
    }
...
const accFilePath = commander.accFile || './.scality_credentials';
...
const config = {
    ...
    simul: {
        ...
        // Return total %cpu usage of all processes found by each pattern
        monitors: 'sproxyd,haproxy',
